...
def _ensure_migrations_table(cursor) -> None:
    # schema_migrations хранит применённые SQL-файлы и защищает их checksums.
    cursor.execute(
        """
...
def run_migrations() -> list[str]:
    ...
    with psycopg2.connect(**settings.resolved_database_config) as connection:
        ...
        for migration_file in sorted(MIGRATIONS_DIR.glob("*.sql")):
            # Миграции добавляются только новыми файлами: уже применённые файлы не переписываем.
